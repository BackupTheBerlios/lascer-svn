...
# Attributs class durch ein Attribut das angibt, ob der Zug gut oder
# perfekt ist.
...
my %neueAttribute = (moveIsGood => '{false, true}');
...
while (<>) {
  ...
  if (/^[\s]*$/) {
    ...
    if (($werte[$attributAnz - 1] eq "good")
        || ($werte[$attributAnz - 1] eq "perfect")) {
      $neueWerte[0] = "true";
    } else {
      $neueWerte[0] = "false";
    }
